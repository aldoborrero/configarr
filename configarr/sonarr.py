"""Sonarr API client using sonarr-py."""
...
# Colon replacement format mapping (integer for Sonarr)
...
# Multi-episode style mapping
MULTI_EPISODE_STYLE_MAP = {
    "extend": 0,
    "duplicate": 1,
    "repeat": 2,
    "scene": 3,
...
class SonarrClient:
    ...
    def __init__(self, base_url: str, api_key: str):
        config = sonarr.Configuration(host=base_url)
    ...
    # Naming Configuration (Sonarr-specific)
    def sync_naming_config(self, config: dict[str, Any]) -> SyncStatus:
        ...
        colon_format = COLON_REPLACEMENT_MAP.get(colon_replacement, 4)

        multi_episode_style = config.get("multi_episode_style", "range")
        multi_episode_int = MULTI_EPISODE_STYLE_MAP.get(multi_episode_style, 5)

        # Build updated config
        updated = NamingConfigResource(
            id=current.id,
            rename_episodes=config.get("rename_episodes", True),
            replace_illegal_characters=config.get("replace_illegal_characters", True),
            colon_replacement_format=colon_format,
            multi_episode_style=multi_episode_int,
            standard_episode_format=config.get(
                "standard_episode_format", current.standard_episode_format
            ),
            daily_episode_format=config.get(
                "daily_episode_format", current.daily_episode_format
            ),
            anime_episode_format=config.get(
                "anime_episode_format", current.anime_episode_format
            ),
            series_folder_format=config.get(
                "series_folder_format", current.series_folder_format
            ),
            season_folder_format=config.get(
                "season_folder_format", current.season_folder_format
            ),
            specials_folder_format=config.get(
                "specials_folder_format", current.specials_folder_format
            ),
        )
    ...
    def sync_release_profile(self, name: str, config: dict[str, Any]) -> SyncStatus:
        """Sync a release profile."""
        existing = self.release_profiles.list_release_profile()
        found = self._find_by_name(existing, name)

        if found:
            log.debug(f"Release profile exists: {name}")
            return SyncStatus.UNCHANGED

        resource = ReleaseProfileResource(
            name=name,
            enabled=config.get("enabled", True),
            required=config.get("required", []),
            ignored=config.get("ignored", []),
            indexer_id=config.get("indexer_id", 0),
            tags=config.get("tags", []),
        )

        self.release_profiles.create_release_profile(release_profile_resource=resource)
    ...
    def _build_fields(
        ...
        from sonarr.models import ContractField
